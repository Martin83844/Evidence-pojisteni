...
const EditInsuredForm = () => {
  ...
  useEffect(() => {
    const fetchInsured = async () => {
      try {
        const response = await axios.get(`/api/Insureds/${id}`);
        setFormData(response.data);
        setLoading(false);
      } catch (error) {
        console.error("Chyba při načítání pojištěného:", error.response?.data || error.message);
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await axios.put(`/api/Insureds/${id}`, formData);
      navigate(`/insureds/${id}`);
    } catch (error) {
      console.error("Chyba při ukládání změn pojištěného:", error.response?.data || error.message);
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="container-fluid px-0">
      <div className="card rounded-0">
        <div className="card-header sticky-header">
          <h1 className="text-center mb-3">Upravit pojištěného</h1>
        </div>
        <div className="cardpositioning p-0">
          <form id="insuredForm" className="form" onSubmit={handleSubmit} noValidate>
...
export default EditInsuredForm;
